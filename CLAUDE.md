...
`CHANGELOG.md` is LOCAL-ONLY: it is gitignored and excluded from the
Quarto render (`project.render` allows only `*.qmd`). It contains
internal notes and must never be committed or published -- it was
briefly live on the public site and was removed on 2026-07-27. Keep it
out of the repo, and keep anything sensitive out of rendered pages;
remember the repo itself is public (GitHub user page).
